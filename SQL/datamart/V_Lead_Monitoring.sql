...
FROM `pi-dev-dashboard.datawarehouse.Leads` 
...
SELECT 
    FORMAT_DATE("%F", date_trunc(created_datetime,Month) ) month
    ,COUNT(distinct id)  cnt
FROM `pi-dev-dashboard.datawarehouse.Opportunity` --SFA.OppotunitySnapshot_*から変更
GROUP BY FORMAT_DATE("%F", date_trunc(created_datetime,Month) )
),
...
FROM `pi-dev-dashboard.master.calender` cal
